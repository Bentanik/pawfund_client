...
export default function MessageComponent() {
  ...
  >([]);
  const [infoParner, setInfoParner] = useState<API.TGetUserNeedSupport | null>(
    null
  );
        ...
        .then(() => {
          ...
          connection.on(
            ...
            (message: TResponseDataHub<API.TMessage>) => {
              handleFetchUserNeedSupport();
              if (infoParner?.UserId == message.Value.Data.SenderId) {
    ...
    >
      <div className="flex items-start relative h-full ">
        ...
        <div className="flex-1 border-2 border-l-[1px] rounded-l-none rounded-lg h-full">
          {infoParner?.UserId === undefined || infoParner?.UserId === null ? (
