...
import usePostBanUser from "../hooks/postBanUser";
import useToast from "@/hooks/use-toast";
import usePostUnBanUser from "../hooks/postUnBanUser";
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export default function ManageUser() {
    const { getAllListUserApi, isPending } = useGetAllListUser();
    const { postBanUserApi } = usePostBanUser();
    const { postUnBanUserApi } = usePostUnBanUser();
    ...
    const [selectedUser, setSelectedUser] = useState<{ id: string; index: number } | null>(null);
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            const response = await getAllListUserApi(params);
    ...
    const handleConfirmBan = async () => {
        if (selectedUser) {
            ...
            const isBlocked = blockedUsers[selectedUser.index];

            if (isBlocked) {
                const response = await postUnBanUserApi({ id: userId });
                if (response) {
                    setBlockedUsers((prev) =>
                        prev.map((blocked, i) => (i === selectedUser.index ? false : blocked))
                    );
                    addToast({ type: "success", description: "User unbanned successfully." });
                    closeModal();
                }
            } else {
                if (banReason) {
                    const response = await postBanUserApi({ id: userId, reason: banReason });
                    if (response) {
                        setBlockedUsers((prev) =>
                            prev.map((blocked, i) => (i === selectedUser.index ? true : blocked))
                        );
                        addToast({ type: "success", description: "User banned successfully." });
                        closeModal();
                    }
                } else {
                    addToast({ type: "error", description: "Ban reason is required." });
                }
            }
        }
    };
                            ...
                            filteredUsers.map((user, index) => (
                                <tr key={user.id} className={`border-b hover:bg-orange-100`}>
                                    ...
                                    <td className="p-3 px-7 text-right">
                                        <button
                                            type="button"
                                            className={`text-base ${blockedUsers[index] ? 'text-green-500 border-green-500' : 'text-red-500 border-red-500'} border py-1 px-2 rounded-full focus:outline-none focus:shadow-outline w-24 bg-transparent font-semibold`}
            ...
            <AlertDialog open={isModalOpen} onOpenChange={setIsModalOpen} >
                <AlertDialogContent className="bg-white">
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={handleConfirmBan}>
                            {blockedUsers[selectedUser?.index ?? -1] ? 'Confirm Unban' : 'Confirm Ban'}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
